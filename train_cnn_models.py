...
def create_model(alpha, kernel_size):
    model = tf.keras.models.Sequential(
        name=f'CNN-Weather-Forecasting-training-{variable}{state}')
    model.add(tf.keras.layers.Input(shape=(73, 144, 1), name='input'))  # input
    # encoder
    model.add(tf.keras.layers.Conv2D(
        filters=16, kernel_size=kernel_size, padding='same', name='conv2D_1'))
    model.add(tf.keras.layers.LeakyReLU(alpha=alpha, name='act_1'))
    model.add(tf.keras.layers.AveragePooling2D(
        pool_size=(2, 2), padding='same', name='AvP_1'))

    model.add(tf.keras.layers.Conv2D(
        filters=32, kernel_size=kernel_size, padding='same', name='conv2D_2'))
    model.add(tf.keras.layers.LeakyReLU(alpha=alpha, name='act_2'))
    model.add(tf.keras.layers.AveragePooling2D(
        pool_size=(2, 2), padding='same', name='AvP_2'))

    model.add(tf.keras.layers.Conv2D(
        filters=64, kernel_size=kernel_size, padding='same', name='conv2D_3'))
    model.add(tf.keras.layers.LeakyReLU(alpha=alpha, name='act_3'))
    model.add(tf.keras.layers.AveragePooling2D(
        pool_size=(2, 2), padding='same', name='AvP_3'))

    model.add(tf.keras.layers.Conv2D(
        filters=128, kernel_size=kernel_size, padding='same', name='conv2D_4'))
    model.add(tf.keras.layers.LeakyReLU(alpha=alpha, name='act_4'))
    model.add(tf.keras.layers.AveragePooling2D(
        pool_size=(2, 2), padding='same', name='AvP_4'))

    # decoder
    model.add(tf.keras.layers.Conv2DTranspose(
        filters=128, kernel_size=kernel_size, padding='same', name='conv2DT_1'))
    model.add(tf.keras.layers.LeakyReLU(alpha=alpha, name='act_5'))
    model.add(tf.keras.layers.UpSampling2D(size=(2, 2), name='UpS2D_1'))

    model.add(tf.keras.layers.Conv2DTranspose(
        filters=64, kernel_size=kernel_size, padding='same', name='conv2DT_2'))
    model.add(tf.keras.layers.LeakyReLU(alpha=alpha, name='act_6'))
    model.add(tf.keras.layers.UpSampling2D(size=(2, 2), name='UpS2D_2'))
    model.add(tf.keras.layers.Cropping2D(
        cropping=((1, 0), (0, 0)), name='Cropping_1'))

    model.add(tf.keras.layers.Conv2DTranspose(
        filters=32, kernel_size=kernel_size, padding='same', name='conv2DT_3'))
    model.add(tf.keras.layers.LeakyReLU(alpha=alpha, name='act_7'))
    model.add(tf.keras.layers.UpSampling2D(size=(2, 2), name='UpS2D_3'))
    model.add(tf.keras.layers.Cropping2D(
        cropping=((1, 0), (0, 0)), name='Cropping_3'))

    model.add(tf.keras.layers.Conv2DTranspose(
        filters=16, kernel_size=kernel_size, padding='same', name='conv2DT_4'))
    model.add(tf.keras.layers.LeakyReLU(alpha=alpha, name='act_8'))
    model.add(tf.keras.layers.UpSampling2D(size=(2, 2), name='UpS2D_4'))
    model.add(tf.keras.layers.Cropping2D(
        cropping=((1, 0), (0, 0)), name='Cropping_4'))

    model.add(tf.keras.layers.Conv2D(filters=1, kernel_size=kernel_size,
              padding='same', activation='linear', name='output'))  # output
...
if __name__ == "__main__":
    ...
    alpha, kernel_size, learning_rate = (best_parameters[[
                                         'params_alpha', 'params_kernel_size',  'params_learning_rate']].values)[0]
