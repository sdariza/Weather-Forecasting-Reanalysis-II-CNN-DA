...
def get_subdomain(r, ip, jp):
    x = []
    j_ = [l % 144 for l in range(jp-r,jp+r+1)] # lons dentro del radio
    if ip == 0:
        i_ = [k % 73 for k in range(ip,ip+r+1)] # lats dentro del radio lat=90
    elif ip == 73 - 1:
        i_ = [k for k in range(ip-r, 73)] # lats dentro del rario lat=-90
    else:
        i_ = [k for k in range(max(0,ip-r), min(73,ip+r+1))] #lats dentro del radio -90<lats<90
    for ii in i_:
        for jj in j_:
            x.append(ii* 144 + jj)
    return x
...
def local_analysis_LEnKF(Xb, H, R, y, N, i, r):
  ...
  if mi>0:
    ...
    Ri = (OBSERVATION_ERROR**2)*np.eye(mi, mi); #Local error covariance matrix
...
for day in range(NUMBER_OF_ASSIMILATION_CYCLES-1):
    ...
    for state in [0, 1, 2, 3]:
        ...
        yz = np.zeros((NUMBER_OF_VARIABLES,))

        #We project back onto the model space (to make easier the indexation)
        yz[H] = y

        Xa = np.zeros((NUMBER_OF_MEMBERS,NUMBER_OF_VARIABLES)); #Local analysis for each model component i
        for i in range(0, NUMBER_OF_VARIABLES):
            Xai = local_analysis_LEnKF(Xb, H, R, y, NUMBER_OF_MEMBERS, i, r)
            Xa[:, i] = Xai[:, r]; #
